...
if "%GNPY_DATA%"=="" (
  echo ERROR: Set GNPY_DATA first.
  exit /b 1
)
...
gnpy-transmission-example "%NETWORK%" --equipment "%EQUIP%" --spectrum "%GNPY_DATA%\multiband_spectrum.json" --show-channels -po 0 > external_validation\gnpy\raw_outputs\gnpy_multiband_builtin_test.txt
if errorlevel 1 (
...
gnpy-transmission-example "%NETWORK%" --equipment "%EQUIP%" --spectrum "%SPECTRUM%" --show-channels -po 0 > external_validation\gnpy\raw_outputs\gnpy_scl_multiband_bandpartition_1span.txt
if errorlevel 1 (
